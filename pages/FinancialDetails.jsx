...
const FinancialDetails = ({ records, onAddRecord, onDeleteRecord, currentYear, sections, payments, totalAnnualSubscriptionIncome }) => {
    ...
    const currentRecords = selectedMonthId !== null ? records.filter(r => {
        ...
        return d.getMonth() === Number(selectedMonthId) && d.getFullYear() === currentYear;
    }) : [];
                    ...
                    >
                        ...
                        <div style={{
                            ...
                            color: (() => {
                                ...
                                const totalYearExpenses = yearRecords.filter(r => r.type === 'expense').reduce((s, r) => s + Number(r.amount), 0);

                                // Total Annual Profit = (Manual Income + Subscription Income) - Expenses
                                const totalAnnualProfit = (totalYearManualIncome + (totalAnnualSubscriptionIncome || 0)) - totalYearExpenses;

                                return totalAnnualProfit >= 0 ? '#4ADE80' : '#F87171';
                            })(),
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'center',
    ...
    const totalExpenses = currentRecords.filter(r => r.type === 'expense').reduce((s, r) => s + Number(r.amount), 0);
    // Final Balance
                    ...
                    <div style={{
                        ...
                        <div style={{ background: '#F0FDF4', padding: '16px', borderRadius: '12px', border: '1px solid #DCFCE7' }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.9rem', color: '#15803d' }}>
                                <span>Από Συνδρομές ({subscriptionCount}):</span>
