...
function App() {
  ...
  const handleAddRecord = (r) => {
    api.addRecord(r).then(newR => setFinancialRecords(p => [newR, ...p])).catch(console.error);
  };
  const handleDeleteRecord = (id) => {
    api.deleteRecord(id).then(() => setFinancialRecords(p => p.filter(x => x.id !== id))).catch(console.error);
  };
  ...
  const { totalExpenses, extraIncome } = useMemo(() => {
    try {
      ...
      const exp = monthRecords.filter(r => r.type === 'expense').reduce((s, r) => s + Number(r.amount || 0), 0);
